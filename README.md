# 债务化解平台 - DDD 架构实践

> 基于领域驱动设计（DDD）的债务化解核心域实现，包含完整的战略设计文档和战术代码实现。  
> **English summary:** A debt settlement core domain implemented with Domain‑Driven Design (DDD), with strategic design docs and production‑style code.

## 关于本仓库 / About this repo

- 以「债务化解」为业务场景，对 DDD 进行系统实践，是我用于学习和面试展示的一个项目。
- 仓库包含：战略设计文档（`docs/`）、核心领域模型与应用代码（`src/`）以及部分单元测试示例。
- 来自线上成熟系统的脱敏版本，保留了核心的领域建模和架构设计思路。

## 我的角色与贡献（Interview Ready）

- 主导债务化解领域的限界上下文划分（分账上下文 / 化债上下文）及聚合设计。
- 设计并实现债务合同、还款计划、分账记录等核心聚合和值对象。
- 引入领域事件、仓储模式、防腐层等 DDD 战术模式，并在项目中落地。
- 为核心领域逻辑补充单元测试，保证模型行为的可验证性与可演进性。
- 对原有传统三层架构进行重构，使复杂业务规则更加清晰、可维护。

## 本地运行 / How to run

### 环境要求

- JDK 17+
- Maven 3.8+
- IDE：推荐 IntelliJ IDEA 或 VS Code（带 Maven 插件）

### 快速开始

```bash
# 运行单元测试，验证领域模型行为
mvn test

# 如需启动应用（如果项目中存在 Spring Boot 启动类）
mvn spring-boot:run
```

## 📖 项目背景

### 业务场景

债务化解平台帮助债务人通过消费行为逐步化解债务：

1. 债务人在商城下单购买商品
2. 支付成功后，平台通过支付网关进行分账（供应商、债务人）
3. 用户确认收货后，债务人分账金额用于化解债务
4. 支持退款场景下的化债回滚

### 原系统痛点

| 问题 | 描述 | 影响 |
|------|------|------|
| 时序耦合 | 分账T+0，结算T+1，确认收货T+7 | 钱到账但债务未化解 |
| 分布式一致性 | 分账在pay模块，化债在crm模块 | 数据可能不一致 |
| 无补偿机制 | 分账成功但化债失败无法恢复 | 资金与债务不匹配 |
| 部分退款困难 | 订单级分账，无法精确回滚 | 退款金额计算混乱 |

### 解决方案

运用 DDD 方法论重新设计化债核心域，实现：

- ✅ 清晰的限界上下文划分（分账上下文、化债上下文）
- ✅ 充血模型，业务逻辑内聚在聚合根
- ✅ 支持商品级精确分账和退款回滚
- ✅ 领域事件驱动，保证最终一致性
- ✅ 防腐层隔离外部系统

## 🏗️ 架构设计

### 限界上下文

```
                    ┌─────────────────────────────────┐
                    │           商城系统               │
                    │        (外部系统)                │
                    └───────────────┬─────────────────┘
                                    │
                          ┌─────────▼─────────┐
                          │    防腐层 (ACL)    │
                          └─────────┬─────────┘
                                    │
                ┌───────────────────┴───────────────────┐
                │                                       │
                ▼                                       ▼
        ┌───────────────┐                       ┌───────────────┐
        │   分账上下文   │                       │   化债上下文   │
        │   (Divide)    │──────────────────────▶│   (Settle)    │
        │               │      提供分账金额      │               │
        └───────┬───────┘                       └───────────────┘
                │
                │ 防腐层
                ▼
        ┌───────────────┐
        │   支付网关     │
        │  (外部系统)    │
        └───────────────┘
```

### 四层架构

```
┌─────────────────────────────────────────────────────────────────────────┐
│                           interfaces（接口层）                           │
│  DebtSettlementController.java   - REST API                            │
│  SettleDebtRequest.java          - 请求 DTO                            │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
┌────────────────────────────────────▼────────────────────────────────────┐
│                          application（应用层）                           │
│  DebtSettlementAppService.java   - 应用服务（编排、事务）                │
│  SettleDebtCommand.java          - 命令对象                             │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
┌────────────────────────────────────▼────────────────────────────────────┐
│                            domain（领域层）★核心★                        │
│  model/                                                                 │
│    ├── shared/       Money, OrderId, Period    - 值对象                 │
│    ├── settle/       DebtContract, RepaymentPlan, RepaymentRecord      │
│    └── divide/       DivideRecord              - 聚合根                 │
│  event/              DebtSettledEvent...       - 领域事件               │
│  repository/         DebtContractRepository    - 仓储接口               │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │
┌────────────────────────────────────▼────────────────────────────────────┐
│                        infrastructure（基础设施层）                      │
│  persistence/        Mapper, DO, Converter     - 持久化实现             │
│  acl/                CaseEntrustQueryService   - 防腐层                 │
└─────────────────────────────────────────────────────────────────────────┘
```

### 聚合设计

```
┌─────────────────────────────────────────────────────────────────┐
│                    DebtContract 聚合（化债合同）                  │
├─────────────────────────────────────────────────────────────────┤
│  DebtContract (聚合根)                                          │
│  ├── id, caseEntrustId, memberUserId                           │
│  ├── totalAmount, paidTotalAmount                              │
│  ├── status: SettleStatus                                      │
│  │                                                              │
│  ├── + settleDebt(orderId, amount, date) : boolean   // 化债   │
│  ├── + rollbackDebt(orderId, amount) : Money         // 回滚   │
│  └── + getAndClearDomainEvents() : List<DomainEvent>           │
│           │                              │                      │
│           │ 1:N                          │ 1:N                  │
│           ▼                              ▼                      │
│  ┌────────────────┐            ┌────────────────────┐          │
│  │ RepaymentPlan  │            │  RepaymentRecord   │          │
│  │ - period       │            │  - orderId         │          │
│  │ - dueAmount    │            │  - amount, type    │          │
│  │ - paidAmount   │            │  - period          │          │
│  └────────────────┘            └────────────────────┘          │
└─────────────────────────────────────────────────────────────────┘

┌─────────────────────────────────────────────────────────────────┐
│                    DivideRecord 聚合（分账记录）                  │
├─────────────────────────────────────────────────────────────────┤
│  DivideRecord (聚合根)                                          │
│  ├── orderId, merchantNo, merchantType                         │
│  ├── divideAmount, refundedAmount, status                      │
│  │                                                              │
│  ├── + markAsDivided(requestNo)              // 标记已分账     │
│  ├── + refund(amount) : Money                // 分账退还       │
│  └── + getEffectiveAmount() : Money          // 有效金额       │
└─────────────────────────────────────────────────────────────────┘
```

## 📁 项目结构

```
debt-settlement-ddd/
├── README.md                          # 本文件
├── docs/                              # 设计文档
│   ├── 01-事件风暴.md                  # 事件风暴、领域事件识别
│   ├── 02-领域模型.md                  # 聚合、实体、值对象设计
│   ├── 03-应用层设计.md                # CQRS、应用服务设计
│   └── 04-防腐层设计.md                # 外部系统隔离策略
│
└── src/                               # 源代码
    └── main/java/.../settle/
        ├── domain/                    # 领域层 ★核心★
        │   ├── model/
        │   │   ├── shared/            # 共享值对象
        │   │   │   ├── Money.java
        │   │   │   ├── OrderId.java
        │   │   │   └── Period.java
        │   │   ├── settle/            # 化债聚合
        │   │   │   ├── DebtContract.java
        │   │   │   ├── RepaymentPlan.java
        │   │   │   ├── RepaymentRecord.java
        │   │   │   └── SettleStatus.java
        │   │   └── divide/            # 分账聚合
        │   │       ├── DivideRecord.java
        │   │       └── DivideStatus.java
        │   ├── event/                 # 领域事件
        │   │   ├── DebtSettledEvent.java
        │   │   ├── DebtRolledBackEvent.java
        │   │   └── ContractCompletedEvent.java
        │   └── repository/            # 仓储接口
        │       ├── DebtContractRepository.java
        │       └── DivideRecordRepository.java
        │
        ├── application/               # 应用层
        │   ├── command/               # 命令
        │   │   ├── SettleDebtCommand.java
        │   │   └── RollbackDebtCommand.java
        │   └── service/               # 应用服务
        │       └── DebtSettlementAppService.java
        │
        ├── infrastructure/            # 基础设施层
        │   ├── persistence/           # 持久化
        │   │   ├── mapper/
        │   │   ├── dataobject/
        │   │   ├── converter/
        │   │   └── repository/
        │   └── acl/                   # 防腐层
        │       ├── CaseEntrustQueryService.java
        │       ├── MemberQueryService.java
        │       └── YeepayDivideService.java
        │
        └── interfaces/                # 接口层
            └── controller/
                └── DebtSettlementController.java
```

## 💡 核心代码示例

### 1. 值对象 - Money（金额）

```java
/**
 * 金额值对象
 * 特点：不可变、自封装、值相等
 */
public final class Money {
    private final long cents;  // 使用分存储，避免浮点精度问题

    public static final Money ZERO = new Money(0L);

    public static Money ofYuan(BigDecimal yuan) {
        long cents = yuan.multiply(BigDecimal.valueOf(100))
                .setScale(0, RoundingMode.HALF_UP)
                .longValue();
        return ofCents(cents);
    }

    public Money add(Money other) {
        return ofCents(this.cents + other.cents);
    }

    public Money subtract(Money other) {
        if (this.cents < other.cents) {
            throw new IllegalArgumentException("金额不足");
        }
        return ofCents(this.cents - other.cents);
    }
}
```

### 2. 聚合根 - DebtContract（化债合同）

```java
/**
 * 债务合同聚合根
 * 职责：管理还款计划、执行化债/回滚、发布领域事件
 */
public class DebtContract {

    private final List<RepaymentPlan> repaymentPlans;
    private final List<RepaymentRecord> repaymentRecords;
    private final List<DomainEvent> domainEvents;

    /**
     * 执行化债（核心业务逻辑）
     */
    public boolean settleDebt(OrderId orderId, Money amount, LocalDate orderCreateTime) {
        // 1. 根据订单时间匹配期数
        Period period = Period.fromDate(orderCreateTime);
        Optional<RepaymentPlan> matchedPlan = findPlanByPeriod(period);

        if (matchedPlan.isEmpty()) {
            // 期数不匹配，发布事件但不报错
            domainEvents.add(new RepaymentPlanNotMatchedEvent(this.id, orderId, period, amount));
            return false;
        }

        // 2. 执行化债
        RepaymentPlan plan = matchedPlan.get();
        Money actualPaid = plan.recordPayment(amount);

        // 3. 更新合同已还总额
        this.paidTotalAmount = this.paidTotalAmount.add(actualPaid);

        // 4. 创建还款记录
        repaymentRecords.add(RepaymentRecord.createIncome(orderId, period, actualPaid));

        // 5. 发布领域事件
        domainEvents.add(new DebtSettledEvent(this.id, orderId, period, actualPaid));

        // 6. 检查合同是否完结
        checkContractCompletion();

        return true;
    }
}
```

### 3. 应用服务 - DebtSettlementAppService

```java
/**
 * 化债应用服务
 * 职责：编排领域对象、事务管理、发布事件
 * 原则：薄服务层，不包含业务逻辑
 */
@Service
public class DebtSettlementAppService {

    @Transactional(rollbackFor = Exception.class)
    public boolean settleDebt(SettleDebtCommand command) {
        // 1. 查询分账金额
        Money divideAmount = divideRecordRepository.getSupplierDivideAmount(command.getOrderNumber());

        // 2. 查询债务合同
        DebtContract contract = debtContractRepository.findByMemberUserId(command.getMemberUserId())
                .orElseThrow(() -> new BusinessException("未找到债务合同"));

        // 3. 执行化债（核心逻辑在聚合根内）
        boolean settled = contract.settleDebt(orderId, divideAmount, command.getOrderCreateTime());

        // 4. 保存合同
        debtContractRepository.save(contract);

        // 5. 发布领域事件
        publishDomainEvents(contract.getAndClearDomainEvents());

        return settled;
    }
}
```

### 4. 单元测试

```java
@DisplayName("DebtContract 聚合根测试")
class DebtContractTest {

    @Test
    @DisplayName("正常化债 - 期数匹配")
    void settleDebt_matchedPeriod_shouldSucceed() {
        // Given
        DebtContract contract = new DebtContract(1L, 100L, Money.ofYuan("12000"));
        contract.addRepaymentPlan(Period.of(2025, 3), Money.ofYuan("1000"));

        // When
        boolean result = contract.settleDebt(
            OrderId.of("ORDER001"),
            Money.ofYuan("500"),
            LocalDate.of(2025, 3, 15)
        );

        // Then
        assertTrue(result);
        assertEquals(Money.ofYuan("500"), contract.getPaidTotalAmount());

        List<DomainEvent> events = contract.getAndClearDomainEvents();
        assertEquals(1, events.size());
        assertInstanceOf(DebtSettledEvent.class, events.get(0));
    }
}
```

## 🎯 设计亮点

### 1. 充血模型 vs 贫血模型

| 贫血模型（传统三层） | 充血模型（DDD） |
|---------------------|----------------|
| 实体只有 getter/setter | 实体包含业务行为 |
| 业务逻辑在 Service 层 | 业务逻辑在聚合根内 |
| 难以保证数据一致性 | 聚合边界保证一致性 |
| 难以单元测试 | 领域层可独立测试 |

### 2. 值对象的价值

- **Money**：封装金额计算逻辑，避免精度问题
- **OrderId**：支持订单级和商品级两种粒度
- **Period**：封装期数（年月）概念，简化匹配逻辑

### 3. 防腐层隔离

```java
// 防腐层接口
public interface CaseEntrustQueryService {
    Optional<Long> getCaseEntrustIdByOrderNumber(String orderNumber);
}

// 领域层不感知外部系统细节
// 外部系统变更只需修改防腐层实现
```

### 4. 领域事件驱动

```java
// 化债完成后发布事件
domainEvents.add(new DebtSettledEvent(contractId, orderId, period, amount));

// 其他模块可订阅事件，实现解耦
@EventListener
public void onDebtSettled(DebtSettledEvent event) {
    // 发送通知、更新统计等
}
```

## 🔗 相关文档

- [01-事件风暴](docs/01-事件风暴.md) - 领域事件识别、命令清单、时序图
- [02-领域模型](docs/02-领域模型.md) - 聚合设计、值对象、领域服务
- [03-应用层设计](docs/03-应用层设计.md) - CQRS、应用服务、DTO 设计
- [04-防腐层设计](docs/04-防腐层设计.md) - 外部系统隔离、接口设计

## 📝 技术栈

- Java 17
- Spring Boot 3.x
- MyBatis Plus
- JUnit 5

## 🙋 关于作者

本项目是我在实际工作中对 DDD 的实践总结。原系统采用传统 MVC 架构，在业务复杂度增长后出现了分布式一致性、代码可维护性等问题。通过 DDD 重构，实现了：

- 业务逻辑内聚，代码可测试性提升
- 清晰的模块边界，团队协作更顺畅
- 支持商品级精确退款，满足业务需求

**注**：本仓库为脱敏后的学习版本，仅展示架构设计思路和核心代码实现。

## 📄 License

MIT License
